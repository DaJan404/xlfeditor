...
export class XlfEditorProvider implements vscode.CustomTextEditorProvider {
    ...
    private async parseXlfContent(content: string): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            parser.parseString(content, (err: any, result: any) => {
                ...
                try {
                    ...
                    const transformed = units.map((unit: any) => {
    ...
    private async updateTranslation(document: vscode.TextDocument, id: string, value: string) {
        try {
            ...
            const unit = transUnits.find((u: any) => u.$.id === id);
            if (!unit) {
    ...
    private async updateTranslations(document: vscode.TextDocument, changes: Array<{id: string, value: string}>) {
        try {
            const xmlContent = document.getText();
            const parser = new xml2js.Parser({
                explicitArray: false,
